...
def real_sol(alpha: float, kappa: float, F: float, f: float, epsil: float, g0: float, g1: float, N: int):
    ...
    if epsil<0.025:
        ...
        A = np.array([[kappa, kappa-alpha*F/epsil],
                      [kappa, kappa*np.exp(F/epsil) + alpha*F/epsil*np.exp(F/epsil)]])
        b = np.array([[g0 + alpha*f/F],
                      [g1 - f/F*(kappa + alpha)]])
...
def train_network(LR: float, HIDDEN: int, NR_EPOCHS: int, h_SAMPLE: float, LAMBDA: float, PLOT_INTERVAL: int, EPSIL: float, Initializer=initializeNetwork, optim='SGD', params=0, momentum=0, M=-1):
    ...
    Result = PINN_solution(HIDDEN) #Create an object from the PINN_solution class
    ...
    #Tensor needed to calculate loss at boundary values, only if alpha is not 0
    ...
    x_0 = x_0.reshape((-1,1))
    all_zeros = np.zeros((int(1/h_SAMPLE), 1)) # this is what the value of -eps*u_xx+u_x-1 should be  
    pt_x0 = torch.autograd.Variable(torch.from_numpy(x_0).float(), requires_grad=True).to(device) #to Tensor
    pt_all_zeros = torch.autograd.Variable(torch.from_numpy(all_zeros).float(), requires_grad=False).to(device) #to Tensor
    ...
    for epoch in range(NR_EPOCHS):
        ...
        optimizer.step(calc_loss) # Optimizing step

        if M>0: # If needed, if a parameter is larger than M, set it back to [-M,M]
